...
lowercase_full_names = [full_names[i].lower() for i in range(len(full_initials))]
...
# numbers =
numbers = [int(almost_numbers[i]) for i in range(len(almost_numbers))]
print(numbers)

# TODO: use a list comprehension to create a list of only the numbers that are
# greater than 9 from the numbers (not strings) you just created
numbers_greaterthan_nine = [numbers[i] for i in range(len(numbers)) if numbers [i] > 9]
print(numbers_greaterthan_nine)